...
public class RedirectServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//문자열 결합만 조심하자 "redirectServlet" + "doGet()"을 올바르지 않다. "redirectServlet {}", "doGet()"
		logger.debug("redirectServlet.doGet()"+"test");
		
		//응답을 다른 jsp에게 위임하는 첫번째 방법 : redirect
		//response 객체의 sendRedirect 메서드를 통해 클라이언트에게 재요청을 보낼 주소를 알려준다
		...
		//http://localhost/delegate/rediectView.jsp로 재요청
		//그래서 최종적으로 클라이언트의 주소줄에는 jsp요청에 대한 주소가 남는다
		//클라이언트 입장에서는 서버로 요청을 총 두번 보내게 된다. => url이 요청한 jsp로 바뀐다.
		...
		//서블릿에서 request 스코프에 설정한 속성은 사용할 수 없다. 때문에 에러발생
		
		//ContextPath : jsp ==> "jsp/delegate/redirextView.jsp"
		//ContextPath : ROOT(/) ==> "/delegate/redirextView.jsp"
