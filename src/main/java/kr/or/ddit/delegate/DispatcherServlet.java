...
public class DispatcherServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		//응답을 다른 jsp(servlet)에게 위임하는 2번째 방법 : Dispatcher
		//request객체의 getRequestDispatcher 메서드를 호출하여 requestDispatcher 객체를 확보
		//				(어떤 jsp, servlet에게 위임할지 인자를 전달)
		//requestDispatcher객체의 forward메서드를 통해 다른 jsp(servlet)에게 응답 생성을 위임
		...
		rd.forward(request, response); // 최조요청을 받은 request,response 인자전달
	}
